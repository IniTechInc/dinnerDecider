...
public final class LlamaClient: LLMClient {
    ...
    /// DinnerDecider fork: drop all KV memory and cached prompt/image chunks so
    /// the next request starts from a clean context. Call between the logical
    /// requests of a long session (e.g. each crop of a fridge scan) to stop
    /// per-request memory accumulating toward a jetsam.
